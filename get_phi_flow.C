...
static const TString Energy[4] = {"200GeV","20GeV","15GeV","7p2GeV",};
static const Int_t pt_total_phi = 17;
...
static const Int_t pt_total_New_phi = 2;
static Float_t pt_low_phi[pt_total_New_phi] =     {0.6,1.2};
static Float_t pt_up_phi[pt_total_New_phi]  =     {1.2,2.4};
static Int_t pt_new_bin_start[pt_total_New_phi] = {2  ,5 };
static Int_t pt_new_bin_stop[pt_total_New_phi]  = {4  ,10 };
...
void get_phi_flow(const Int_t energy = 3)
{
    ...
    TString inputfile_SE = Form("data/out_PhiMesonAna_%s.root",Energy[energy].Data());
  ...
  // TH1F *h_mMass_Phi3_SE[pt_total_phi][Centrality_total][Phi_Psi_total];
    ...
    // TH1F *h_mMass_Phi2_ME[pt_total_phi][Centrality_total][Phi_Psi_total];
          ...
          for(Int_t X_flag = 0; X_flag < 2; X_flag++) // x_flag
            {
                for(Int_t m = 0; m < Phi_Psi_total; m++) // phi-psi bin
                {
                  ...
                  h_mMass_Phi2_Ana[i][j][X_flag][m] = (TH1F*)File_Ana->Get(HistName.Data())->Clone();
                    ...
                    // h_mMass_Phi2_SE[i][j][m] = (TH1F*)File_Ana->Get(HistName.Data())->Clone();
                    // HistName = Form("pt_%d_Centrality_%d_phi_Psi_%d_3rd_Phi_SE",i,j,m);
                    // h_mMass_Phi3_SE[i][j][m] = (TH1F*)File_Ana->Get(HistName.Data())->Clone();

                    // HistName = Form("pt_%d_Centrality_%d_phi_Psi_%d_2nd_Phi_ME",i,j,m);
                    // h_mMass_Phi2_ME[i][j][m] = (TH1F*)File_Ana->Get(HistName.Data())->Clone();
                    // HistName = Form("pt_%d_Centrality_%d_phi_Psi_%d_3rd_Phi_ME",i,j,m);
                    // h_mMass_Phi3_ME[i][j][m] = (TH1F*)File_Ana->Get(HistName.Data())->Clone();
                }
            }
        }
    }
    ...
    TH1F *h_mMass_Phi2_SM[pt_total_phi][Centrality_total][Phi_Psi_total];
    ...
    TCanvas *c1_invmass[pt_total_New_phi][Centrality_total][Phi_Psi_total];
    char name[80];
    for(Int_t j = Centrality_start; j < Centrality_stop; j++) // centrality bin
    {
        // for(Int_t l = EtaGap_start; l < EtaGap_stop; l++) // eta gap bin
        // {
            for(Int_t m = 0; m < Phi_Psi_total; m++) // phi-psi bin
            {
                for(Int_t i = 0; i < pt_total_New_phi; i++)
                {
                    ...
                    h_mMass_Phi2_SM_new_bin[i][j][m]->Add(h_mMass_Phi2_Ana_New_Bin[i][j][1][m],-1.0);

                    sprintf(name, "c1_invmass_Pt%d_Cent%d_phi_Psi%d",i,j,m);
                    c1_invmass[i][j][m] = new TCanvas(name, name, 600, 500);
                    c1_invmass[i][j][m]->cd();
    ...
    TH1F *h_mMass_Phi2_SM_New_Bin[pt_total_New_phi][Centrality_total][Phi_Psi_total];
    TH1F *h_mMass_Phi3_SM_New_Bin[pt_total_New_phi][Centrality_total][Phi_Psi_total];
            ...
            for(Int_t m = 0; m < Phi_Psi_total; m++) // phi-psi bin
            {
                for(Int_t i = 0; i < pt_total_New_phi; i++)
                {
                    for(Int_t pt_bin = pt_new_bin_start[i]; pt_bin <= pt_new_bin_stop[i]; pt_bin++)
                    {
                        ...
                        {
                            h_mMass_Phi2_SM_new_bin[i][j][m]->Add(h_mMass_Phi2_SM[pt_bin][j][m],1.0);
                            // h_mMass_Phi3_SM_New_Bin[i][j][m]->Add(h_mMass_Phi3_SM[pt_bin][j][m],1.0);
                        }
                    }
                    //if(i >= 6)
                    {
                        h_mMass_Phi2_SM_new_bin[i][j][m]->Rebin(4);
            ...
            // {
                for(Int_t m = 0; m < Phi_Psi_total; m++) // phi-psi bin
                {
                    ...
                    if(m == 0)
                    {
                        ...
                        h_mMass_Phi2_SM_total[i][j] = (TH1F*)h_mMass_Phi2_SM_new_bin[i][j][m]->Clone(HistName.Data());
                    ...
                    {
                        h_mMass_Phi2_SM_total[i][j]->Add(h_mMass_Phi2_SM_new_bin[i][j][m],1.0);
    ...
    TF1 *f_PolyBW_Phi2[pt_total_New_phi][Centrality_total][Phi_Psi_total];
    TF1 *f_PolyBW_Phi3[pt_total_New_phi][Centrality_total][Phi_Psi_total];
    for(Int_t i = 0; i < pt_total_New_phi; i++) // pt bin
    {
        for(Int_t j = Centrality_start; j < Centrality_stop; j++) // centrality bin
        {
            ...
            // {
                for(Int_t m = 0; m < Phi_Psi_total; m++) // phi-psi bin
                {
                    ...
                    f_PolyBW_Phi2[i][j][m] = new TF1(HistName.Data(),PolyBreitWigner,BW_Start,BW_Stop,5);
                    for(Int_t n_par = 0; n_par < 5; n_par++)
                    {
                        f_PolyBW_Phi2[i][j][m]->ReleaseParameter(n_par);
                    }
                    f_PolyBW_Phi2[i][j][m]->FixParameter(0,ParFit2_total[i][j][0]);
                    f_PolyBW_Phi2[i][j][m]->FixParameter(1,ParFit2_total[i][j][1]);
                    f_PolyBW_Phi2[i][j][m]->SetParameter(2,ParFit2_total[i][j][2]/7.0);
                    f_PolyBW_Phi2[i][j][m]->SetParameter(3,ParFit2_total[i][j][3]);
                    f_PolyBW_Phi2[i][j][m]->SetParameter(4,ParFit2_total[i][j][4]);
                    f_PolyBW_Phi2[i][j][m]->SetRange(BW_Start,BW_Stop);
                    /*
                    ...
                    */
                    h_mMass_Phi2_SM_new_bin[i][j][m]->Fit(f_PolyBW_Phi2[i][j][m],"RMN");
                    // h_mMass_Phi3_SM_New_Bin[i][j][m]->Fit(f_PolyBW_Phi3[i][j][m],"RMN");
                }
            // }
        }
    }
    ...
    TF1 *f_Poly_Phi2[pt_total_New_phi][Centrality_total][Phi_Psi_total];
        ...
        for(Int_t j = Centrality_start; j < Centrality_stop; j++) // centrality bin
        {
            ...
            // {
                for(Int_t m = 0; m < Phi_Psi_total; m++) // phi-psi bin
                {
                    ...
                    f_Poly_Phi2[i][j][m] = new TF1(FuncName.Data(),Poly,BW_Start,BW_Stop,2);
                    f_Poly_Phi2[i][j][m]->FixParameter(0,f_PolyBW_Phi2[i][j][m]->GetParameter(3));
                    f_Poly_Phi2[i][j][m]->FixParameter(1,f_PolyBW_Phi2[i][j][m]->GetParameter(4));
                    h_mMass_Phi2_SM_new_bin[i][j][m]->Add(f_Poly_Phi2[i][j][m],-1.0);
                    /*
        ...
        for(Int_t j = Centrality_start; j < Centrality_stop; j++) // centrality bin
        {
            ...
            // {
                ...
                for(Int_t m = 0; m < Phi_Psi_total; m++)
                {
                    if(m == 0)
                    {
                        h_mMass_Phi2_Sig_total[i][j] = (TH1F*)h_mMass_Phi2_SM_new_bin[i][j][m]->Clone(HistName.Data());
                        h_mMass_Phi2_Sig_total[i][j]->SetTitle("");
                        f_gauss_2[i][j] = new TF1(FuncName_g.Data(),Gaussion,BW_Start,BW_Stop,3);
                        f_gauss_2[i][j]->SetParameter(0,ParFit2_total[i][j][0]);
                        //	    f_gauss_2[i][j]->SetParameter(1,ParFit2_total[i][j][1]);
                        f_gauss_2[i][j]->SetParameter(1,0.07);
                        f_gauss_2[i][j]->SetParameter(2,1000);
                        f_gauss_2[i][j]->SetRange(ParFit2_total[i][j][0]-gaussSig*ParFit2_total[i][j][1],ParFit2_total[i][j][0]+gaussSig*ParFit2_total[i][j][1]);
                    ...
                    {
                        h_mMass_Phi2_Sig_total[i][j]->Add(h_mMass_Phi2_SM_new_bin[i][j][m],1.0);
                    }
                }
                h_mMass_Phi2_Sig_total[i][j]->Fit(f_gauss_2[i][j],"MQNR");
                if(i == 8 && j ==3) {h_mMass_Phi2_Sig_total[i][j]->Fit(f_gauss_2[i][j],"MQNR");}
        ...
        for(Int_t j = Centrality_start; j < Centrality_stop; j++) // centrality bin
        {
            ...
            // {
                ...
                for(Int_t m = 0; m < Phi_Psi_total; m++) // phi-psi bin
                {
                    ...
                    h_mMass_Phi2_SM_new_bin[i][j][m]->SetTitle("");
                    h_mMass_Phi2_SM_new_bin[i][j][m]->SetNdivisions(505,"X");
                    h_mMass_Phi2_SM_new_bin[i][j][m]->SetNdivisions(505,"Y");
                    h_mMass_Phi2_SM_new_bin[i][j][m]->GetXaxis()->SetLabelSize(0.05);
                    h_mMass_Phi2_SM_new_bin[i][j][m]->GetYaxis()->SetLabelSize(0.05);
                    h_mMass_Phi2_SM_new_bin[i][j][m]->GetXaxis()->SetTitle("M(K^{+},K^{-}) (GeV/c^{2})");
                    h_mMass_Phi2_SM_new_bin[i][j][m]->GetYaxis()->SetTitle("Counts");
                    h_mMass_Phi2_SM_new_bin[i][j][m]->GetXaxis()->SetTitleSize(0.06);
                    h_mMass_Phi2_SM_new_bin[i][j][m]->GetYaxis()->SetTitleSize(0.06);
                    h_mMass_Phi2_SM_new_bin[i][j][m]->GetYaxis()->SetTitleOffset(1.7);
                    h_mMass_Phi2_SM_new_bin[i][j][m]->GetXaxis()->CenterTitle();
                    h_mMass_Phi2_SM_new_bin[i][j][m]->GetYaxis()->CenterTitle();
                    h_mMass_Phi2_SM_new_bin[i][j][m]->Draw("pE");
                    ...
                    Float_t y = h_mMass_Phi2_SM_new_bin[i][j][m]->GetBinContent(h_mMass_Phi2_SM_new_bin[i][j][m]->FindBin(ParGaus_2[i][j][0]));
        ...
        for(Int_t j = Centrality_start; j < Centrality_stop; j++) // centrality bin
        {
            ...
            // {
                ...
                for(Int_t m = 0; m < Phi_Psi_total; m++) // phi-psi bin
                {
                    ...
                    Int_t bin_start = h_mMass_Phi2_SM_new_bin[i][j][m]->FindBin(ParGaus_2[i][j][0]-nSigmaPhi*ParGaus_2[i][j][1]);
                    Int_t bin_stop  = h_mMass_Phi2_SM_new_bin[i][j][m]->FindBin(ParGaus_2[i][j][0]+nSigmaPhi*ParGaus_2[i][j][1]);
                    if(i ==8 && j==3)
                    {
                    ...
                    {
                        counts += h_mMass_Phi2_SM_new_bin[i][j][m]->GetBinContent(bin);
                        errors += h_mMass_Phi2_SM_new_bin[i][j][m]->GetBinError(bin)*h_mMass_Phi2_SM_new_bin[i][j][m]->GetBinError(bin);
                    }
    ...
    for(Int_t i = 0; i < pt_total_New_phi; i++)
    {
        for(Int_t j = Centrality_start; j < Centrality_stop; j++) // centrality bin
        {
            ...
            // {
                ...
                h_Counts2[i][j]->GetYaxis()->SetRangeUser(h_Counts2[i][j]->GetBinContent(bin_low)*0.9,h_Counts2[i][j]->GetBinContent(bin_up)*1.1);
    ...
    TF1 *f_bw_phi_2[pt_total_New_phi][Centrality_total][Phi_Psi_total];
    TF1 *f_bw_phi_3[pt_total_New_phi][Centrality_total][Phi_Psi_total];
        ...
        for(Int_t j = Centrality_start; j < Centrality_stop; j++) // centrality bin
        {
            // for(Int_t l = EtaGap_start; l < EtaGap_stop; l++) // eta gap bin
            {
                ...
                for(Int_t m = 0; m < Phi_Psi_total; m++) // phi-psi bin
                {
                    ...
                    f_bw_phi_2[i][j][m] = new TF1(FuncName_bw.Data(),BreitWigner,BW_Start,BW_Stop,3);;
                    f_bw_phi_2[i][j][m]->FixParameter(0,ParBW_2[i][j][0]);
                    f_bw_phi_2[i][j][m]->FixParameter(1,ParBW_2[i][j][1]);
                    f_bw_phi_2[i][j][m]->SetParameter(2,1000);
                    f_bw_phi_2[i][j][m]->SetRange(BW_Start,BW_Stop);
                    h_mMass_Phi2_SM_new_bin[i][j][m]->Fit(f_bw_phi_2[i][j][m],"NMQR");
                    Float_t bin_width = h_mMass_Phi2_SM_new_bin[i][j][m]->GetBinWidth(1);
                    ...
                    Float_t counts = f_bw_phi_2[i][j][m]->Integral(Inte_start,Inte_stop)/bin_width;
                    Float_t errors = f_bw_phi_2[i][j][m]->IntegralError(Inte_start,Inte_stop)/bin_width;
                    Float_t bin_center = PI_max[0]/14.0+m*PI_max[0]/7.0;
        ...
        for(Int_t j = Centrality_start; j < Centrality_stop; j++) // centrality bin
        {
            // for(Int_t l = EtaGap_start; l < EtaGap_stop; l++) // eta gap bin
            {
                ...
                for(Int_t m = 0; m < Phi_Psi_total; m++) // phi-psi bin
                {
                    ...
                    h_mMass_Phi2_SM_new_bin[i][j][m]->Draw("pE");
                    f_bw_phi_2[i][j][m]->SetLineColor(2);
                    f_bw_phi_2[i][j][m]->Draw("l Same");
                    Float_t x1 = ParGaus_2[i][j][0] - nSigmaPhi*ParGaus_2[i][j][1];
                    Float_t x2 = ParGaus_2[i][j][0] + nSigmaPhi*ParGaus_2[i][j][1];
                    Float_t y = h_mMass_Phi2_SM_new_bin[i][j][m]->GetBinContent(h_mMass_Phi2_SM_new_bin[i][j][m]->FindBin(ParGaus_2[i][j][0]));
                    PlotLine(x1,x1,0,y,4,2,2);
    ...
    for(Int_t i = 0; i < pt_total_New_phi; i++)
    {
        for(Int_t j = Centrality_start; j < Centrality_stop; j++) // centrality bin
        {
            for(Int_t l = EtaGap_start; l < EtaGap_stop; l++) // eta gap bin
            {
                ...
                h_Counts2_bw[i][j]->GetYaxis()->SetRangeUser(h_Counts2_bw[i][j]->GetBinContent(bin_low)*0.9,h_Counts2_bw[i][j]->GetBinContent(bin_up)*1.1);
    ...
    */
    ...
    for(Int_t j = Centrality_start; j < Centrality_stop; j++) // centrality bin
    {
        //for(Int_t l = EtaGap_start; l < EtaGap_stop; l++) // eta gap bin
        {
            ...
            for(Int_t i = 0; i < pt_total_New_phi; i++)
            {
                Int_t bin_center = h_flow_2[j]->FindBin((pt_low_phi[i]+pt_up_phi[i])/2.0);
            ...
            */
            ...
            for(Int_t i = 0; i < pt_total_New_phi; i++)
            {
                Int_t bin_center = h_flow_2_bw[j]->FindBin((pt_low_phi[i]+pt_up_phi[i])/2.0);
    ...
    for(Int_t j = 0; j < 9; j++) // centrality bin
    {
        //for(Int_t l = EtaGap_start; l < EtaGap_stop; l++) // eta gap bin
        {
            ...
            HistName = Form("Yields_Centrality_%d_Phi_SE",j);
            h_Yields_Phi_SE[j] = (TH1F*)File_Ana->Get(HistName.Data())->Clone();

            HistName = Form("Yields_Centrality_%d_Phi_ME",j);
            h_Yields_Phi_ME[j] = (TH1F*)File_Ana->Get(HistName.Data())->Clone();
        }
    }
    ...
    for(Int_t j = 0; j < 9; j++) // centrality bin
    {
        //for(Int_t l = EtaGap_start; l < EtaGap_stop; l++) // eta gap bin
        {
            ...
            for(Int_t bin = bin_start; bin <= bin_stop; bin++)
            {
                Counts_Yields[j] += h_Yields_Phi_SM[j]->GetBinContent(bin);
            }
        }
    }

    // get resolution
    TString input_res = Form("data/Resolution.root");
